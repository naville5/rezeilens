...
(function(html) {
    ...
    * ---------------------------------------------------- */ 
    const sstabs = function(nextTab = false) {
        ...
        //document.getElementById('getStartedNow').addEventListener('click', function() { });
        ...
        // Listen for the button click and navigate to the Services tab
        document.getElementById('linkOurServices').addEventListener('click', function() {
            // Find the index of the tab with id 'tab-services'
            //console.log("linkOurServices")
            const servicesTabLink = Array.from(tabLinks).find(link => link.getAttribute('href') === '#tab-services');
            const index = Array.from(tabLinks).indexOf(servicesTabLink);
        
            if (index !== -1) {
                tabClickEvent(servicesTabLink, tabLinks, tabPanels, index);
            }
        });
        
        // Listen for the button click and navigate to the Services tab
        document.getElementById('linkGetStartedNow').addEventListener('click', function() {
            // Find the index of the tab with id 'tab-services'
            //console.log("linkGetStartedNow")
            const servicesTabLink = Array.from(tabLinks).find(link => link.getAttribute('href') === '#tab-services');
            const index = Array.from(tabLinks).indexOf(servicesTabLink);
